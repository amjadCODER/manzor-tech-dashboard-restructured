...
export default function SettingsPage() {
  ...
  async function chooseUser(user: UserRecord) {
    ...
    const data = await select<UserPermission[]>(
      "user_permissions",
      `select=app_key&user_id=eq.${user.id}`,
    );
  ...
  async function deleteUser() {
    ...
    const confirmed = window.confirm(
      `هل انت متأكد من حذف حساب ${selected.full_name || selected.email}؟ هذا الاجراء لا يمكن التراجع عنه.`,
    );
    ...
    <main className="settings-shell">
      <header className="workspace-header">
        <div>
          ...
          <p>
            ادارة الحسابات وحالتها وصلاحيات الانظمة وكلمات المرور من مكان واحد.
          </p>
        </div>

        <Link className="platform-nav-btn" href="/platform">
          العودة للمنصة
        </Link>
      </header>
            ...
            {filteredUsers.map((user) => (
              ...
              >
                <span className="user-avatar">
                  {user.full_name?.trim()?.charAt(0) || "م"}
                </span>
                ...
                <span className={`status-pill ${user.status}`}>
                  {user.status === "active" ? "فعال" : "موقوف"}
                </span>
              </button>
            ))}

            {!filteredUsers.length && (
              <div className="empty-state compact">لا توجد نتائج</div>
            )}
          </div>
        </section>
          ...
          {!selected ? (
            <div className="empty-state">
              <div>
                <h2>اختر مستخدما</h2>
                <p>
                  اختر حسابا من القائمة لتعديل الدور والصلاحيات او كلمة المرور.
                </p>
              </div>
            </div>
          ) : (
            <>
              <div className="user-editor-heading">
                <div>
                  <span className="workspace-kicker">USER ACCESS</span>
                  <h2>{selected.full_name || "بدون اسم"}</h2>
                  <p dir="ltr">{selected.email}</p>
                </div>

                <span className={`status-pill ${selected.status}`}>
                  {selected.status === "active" ? "فعال" : "موقوف"}
                </span>
              </div>
              ...
              <div className="editor-section">
                <div className="section-copy">
                  ...
                  <p>
                    حدد الانظمة التي تظهر لهذا المستخدم. الادمن يملك وصولا كاملا
                    تلقائيا.
                  </p>
                </div>
                  ...
                  {APPS.filter((app) => app.key !== "settings").map((app) => (
                    <label className="perm" key={app.key}>
                      <input
                        ...
                        checked={
                          selected.role === "admin" ||
                          permissions.includes(app.key)
                        }
                        onChange={(event) =>
                          setPermissions(
                            event.target.checked
                              ? [...permissions, app.key]
                              : permissions.filter((key) => key !== app.key),
                          )
                        }
                      />

                      <span>
                        <strong>{app.ar}</strong>
                        <small>{app.en}</small>
                      </span>
                    </label>
                  ))}
              ...
              <div className="editor-section password-section">
                <div className="section-copy">
                  ...
                  <p>
                    اترك الحقل فارغا اذا لم ترغب في تغييرها. يجب الا تقل كلمة
                    المرور عن 8 احرف.
                  </p>
                </div>
              ...
              <div className="editor-actions">
                ...
                <button
                  ...
                  onClick={saveUser}
                >
                  {busy ? "جاري التنفيذ" : "حفظ التعديلات"}
                </button>
              </div>
            </>
          )}
